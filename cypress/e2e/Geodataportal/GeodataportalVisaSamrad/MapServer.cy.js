...
describe("Geodataportal/GeodataportalVisaSamrad", () => {
    URLSUFIX.forEach((urlsf) => {
        ...
        Cypress._.times(RERUNS, (index) => {
            it(`MapServer - ${KarttjansURL}/services/Geodataportal/GeodataportalVisaSamrad/MapServer?f=pjson - Run ${index + 1}`, () => {
                cy.fixture('karttjanster/Geodataportal/GeodataportalVisaSamrad/MapServer').then((fixture) => {
                    cy.log('Fixture data:', JSON.stringify(fixture));
                    cy.request({
                        method: 'GET',
                        url: `${KarttjansURL}/services/Geodataportal/GeodataportalVisaSamrad/MapServer?f=pjson`,
                        headers: {
