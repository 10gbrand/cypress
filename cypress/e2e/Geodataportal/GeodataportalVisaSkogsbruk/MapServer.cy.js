...
describe("Geodataportal/GeodataportalVisaSkogsbruk", () => {
    URLSUFIX.forEach((urlsf) => {
        ...
        Cypress._.times(RERUNS, (index) => {
            it(`MapServer - ${KarttjansURL}/services/Geodataportal/GeodataportalVisaSkogsbruk/MapServer?f=pjson - Run ${index + 1}`, () => {
                cy.fixture('karttjanster/Geodataportal/GeodataportalVisaSkogsbruk/MapServer').then((fixture) => {
                    cy.log('Fixture data:', JSON.stringify(fixture));
                    cy.request({
                        method: 'GET',
                        url: `${KarttjansURL}/services/Geodataportal/GeodataportalVisaSkogsbruk/MapServer?f=pjson`,
                        headers: {
