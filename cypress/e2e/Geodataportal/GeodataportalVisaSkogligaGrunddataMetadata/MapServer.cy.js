...
describe("Geodataportal/GeodataportalVisaSkogligaGrunddataMetadata", () => {
    URLSUFIX.forEach((urlsf) => {
        ...
        Cypress._.times(RERUNS, (index) => {
            it(`MapServer - ${KarttjansURL}/services/Geodataportal/GeodataportalVisaSkogligaGrunddataMetadata/MapServer?f=pjson - Run ${index + 1}`, () => {
                cy.fixture('karttjanster/Geodataportal/GeodataportalVisaSkogligaGrunddataMetadata/MapServer').then((fixture) => {
                    cy.log('Fixture data:', JSON.stringify(fixture));
                    cy.request({
                        method: 'GET',
                        url: `${KarttjansURL}/services/Geodataportal/GeodataportalVisaSkogligaGrunddataMetadata/MapServer?f=pjson`,
                        headers: {
